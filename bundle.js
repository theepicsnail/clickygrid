...
var Chunk = (function () {
     ...
     * @param y
     */
    Chunk.prototype.clicked = function (x, y) { };
    return Chunk;
}());
exports.Chunk = Chunk;
var FirebaseChunk = (function (_super) {
    __extends(FirebaseChunk, _super);
    function FirebaseChunk(x, y) {
        var _this = _super.call(this, x, y) || this;
        _this.drawText("Loading...", "#000");
        globals.game.chunkManager.publishEvent("update", _this);
        _this.ref = firebase.database().ref("/chunks/" + _this.x + "," + _this.y);
        _this.ref.on('value', function (v) { _this.dbUpdate(v.val()); });
        return _this;
    }
    FirebaseChunk.prototype.dbUpdate = function (update) {
        var _this = this;
        if (update === null) {
            update = {};
        }
        this.values = {};
        for (var y = 0; y < globals.blockSize; y++)
            for (var x = 0; x < globals.blockSize; x++) {
                var key = x + "," + y;
                this.values[key] = update[key] ||
                    globals.mapDefault(this.x * globals.blockSize + x, this.y * globals.blockSize + y);
            }
        Promise
            .race([
            new Promise(function (res, rej) { setTimeout(rej, 100); }),
            globals.game.resources.getTilesheet("res/tiles.png")
                .then(this.drawTiles.bind(this))
                .catch(function () { _this.drawText("Failed", "#800"); })
                .then(function () {
                globals.game.chunkManager.publishEvent("update", _this);
                return true;
            })
    ...
    FirebaseChunk.prototype.drawTiles = function (tiles) {
        for (var y = 0; y < globals.blockSize; y++)
            for (var x = 0; x < globals.blockSize; x++) {
                var val = this.values[x + "," + y]['value'];
                globals.game.resources.drawTile(tiles, val, this.ctx, x * globals.tileSize, y * globals.tileSize);
            }
        globals.game.chunkManager.publishEvent("update", this);
    };
    FirebaseChunk.prototype.drawText = function (text, color) {
    ...
    FirebaseChunk.prototype.unload = function () {
        _super.prototype.unload.call(this);
        this.ref.off();
        return true;
    };
    FirebaseChunk.prototype.clicked = function (x, y) {
        var def = globals.mapDefault(this.x * globals.blockSize + x, this.y * globals.blockSize + y);
        var cur = this.values[x + "," + y]['value'];
        var next = globals.terrain.GRASS;
        switch (cur) {
...
var TerrainDevChunk = (function (_super) {
    __extends(TerrainDevChunk, _super);
    function TerrainDevChunk(x, y) {
        var _this = _super.call(this, x, y) || this;
        _this.render();
        return _this;
    }
    TerrainDevChunk.prototype.render = function () {
        var id = this.ctx.createImageData(globals.pixelsPerChunk, globals.pixelsPerChunk);
        var data = id.data;
        for (var x = 0; x < globals.pixelsPerChunk; x++)
            for (var y = 0; y < globals.pixelsPerChunk; y++) {
                var o = (x + y * globals.pixelsPerChunk) * 4;
                var d = globals.mapDefault(this.x * globals.pixelsPerChunk + x, this.y * globals.pixelsPerChunk + y);
                var color = [255, 0, 0];
                switch (d.value) {
                    case globals.terrain.GOLD:
                        color = [255, 128, 0];
    ...
    TerrainDevChunk.prototype.unload = function () { return false; };
    return TerrainDevChunk;
}(Chunk));
...
Object.defineProperty(exports, "__esModule", { value: true });
var DebugProperties = (function (_super) {
    __extends(DebugProperties, _super);
...
var Debug = (function () {
    function Debug() {
        this.div_ = document.getElementById("debug");
        this.updateRequested_ = false;
        this.toUpdate_ = new Set();
        this.values_ = new DebugProperties();
        this.proxy_ = new Proxy(this.values_, this);
    }
    Debug.prototype.getProxyObject = function () {
        return this.proxy_;
    };
    Debug.prototype.get = function (target, name, receiver) {
        if (!target.hasOwnProperty(name))
            return false;
        return target[name].value;
    };
    Debug.prototype.set = function (target, name, value, receiver) {
        if (target.hasOwnProperty(name)) {
    ...
    Debug.prototype.deleteProperty = function (target, name) {
        if (!target.hasOwnProperty(name))
            return false;
    ...
    Debug.prototype.createPre = function () {
        var pre = document.createElement("pre");
        this.div_.appendChild(pre);
        return pre;
    };
    Debug.prototype.applyChanges = function () {
        var _this = this;
        this.toUpdate_.forEach(function (v) {
            var record = _this.values_[v];
 ...
 */
var chunkManager_1 = __webpack_require__(2);
var camera_1 = __webpack_require__(1);
var controls_1 = __webpack_require__(3);
var resources_1 = __webpack_require__(5);
var user_1 = __webpack_require__(6);
var firebase_1 = __webpack_require__(4);
firebase_1.initFirebase(function (user, ref) {
